...
class MultiThreadCell(threading.Thread):
    def __init__(
        ...
        self.cell_type_dict = {"Bubble": 0, "Selection": 1, "Insertion": 2}
    ...
    def take_snapshot(self):
        return [c.value for c in self.cells], [
            [
                c.group.group_id,
                self.cell_type_dict[c.cell_type] if c.label == 0 else c.label,
                c.value,
    ...
    def get_current_snapshot(self):
        return {
            "value": self.value,
            "group id": self.group.group_id,
            "group status": self.group.status,
    ...
    def swap(self, target_position, skip_stats=False):
        ...
        if self.visualization_disabled:
            ...
            current_cell_at_target.current_position = (
                current_cell_at_target.target_position
            )
