...
class CellGroup(threading.Thread):
    ...
    def is_group_sorted(self):
        ...
        for i in range(
            self.left_boundary_position[0], self.right_boundary_position[0] + 1
        ):
            cell = self.global_cells[i]
            if (
                cell.status == CellStatus.SLEEP
                or cell.status == CellStatus.MOVING
                or cell.value < prev_cell.value
            ):
                return False
            prev_cell = cell
        return True
    ...
    def is_boundary(self):
        ...
        return (
            checking_next_index >= len(self.global_cells)
            or self.group_id != self.global_cells[checking_next_index].group_id
        )
        ...
        while self.status != GroupStatus.MERGED and not self.all_cells_inactive():
            ...
            if self.status == GroupStatus.ACTIVE:
                ...
                if self.status == GroupStatus.ACTIVE and self.is_group_sorted():
                    ...
                    if (
                        next_group
                        and next_group.status == GroupStatus.ACTIVE
                        and next_group.is_group_sorted()
                    ):
