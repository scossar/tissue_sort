    ...
    def should_move(self):
        if self.reverse_direction:
            ...
            if self.current_position[0] > self.left_boundary[0]:
                bigger_than_left = (
                    self.value > self.cells[int(self.current_position[0] - 1)].value
                    and self.cells[int(self.current_position[0] - 1)].status
                    == CellStatus.ACTIVE
                )
    ...
    def should_move_to(self, target_position, check_right):
        if (
            ...
            and (
                ...
                or self.cells[int(target_position[0])].status == CellStatus.FREEZE
            )
        ):
            ...
            if self.reverse_direction:
                return (
                    self.value < self.cells[int(target_position[0])].value
                    if check_right
                    else self.value > self.cells[int(target_position[0])].value
                )
            return (
                self.value > self.cells[int(target_position[0])].value
                if check_right
